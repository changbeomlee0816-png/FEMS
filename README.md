...
- **`기호 해설` 탭** — 60종 기호의 도면 읽는 법, ANSI 기기번호 28종, AF/AT·%Z 같은 표기,
  에너지원 8종을 검색해 볼 수 있습니다.
- **차단기를 클릭하면 투입/개방**이 바뀌고 그 아래 계통이 즉시 정전으로 표시됩니다.
  **모선 연락(TIE)** 을 투입하면 반대편 수전에서 되살아납니다.
...
- **양식을 화면에서 바로 내려받습니다.** 스키마에서 생성하므로 검증기와 어긋나지 않습니다.
  쓰이지 않던 항목(홈페이지·건물규모·파워플래너 비밀번호 등)을 걷어내 **기본정보 13칸,
  장비 10열**로 줄였습니다.
...
- 화면 하단 **알람·이벤트 바**(미확인/활성 건수, 확인 처리)와 **구역 탭**,
  도면 안 **표제란·심볼 범례**를 갖췄습니다.
